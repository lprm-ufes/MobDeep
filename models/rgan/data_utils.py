...
def get_samples_and_labels(settings):
    ...
    dictionaries.
    """
    print ('Predict labels', settings['predict_labels'], settings["normalise"])
    if settings['data_load_from']:
        data_path = settings['datadir'] + settings['data_load_from'] + '.data.npy'
...
def get_data(data_type, data_options=None):
    ...
    if data_type == 'load':
        ...
        print(data_options)
        samples, pdf = GP(kernel='rbf', **data_options)
