...
def visualise_at_epoch(vis_sample, data, predict_labels, one_hot, epoch,
        ...
        seq_length, labels, exp_id, test_id):
    ...
    if data == 'mnist':
        ...
        save_plot_sample(vis_sample, epoch, identifier, n_samples=6,
                num_epochs=num_epochs, exp_id=exp_id, test_id=test_id)

    return True

def save_plot_sample(samples, idx, identifier, n_samples=6, num_epochs=None, ncol=2, exp_id="", test_id=""):
    ...
    path_training_plots = "experiments/{}/{}/evaluations/training_plots/".format(exp_id, test_id)
    try:
        os.makedirs(path_training_plots)
    except:
        pass
    fig.savefig("{}{}_epoch{}.png".format(path_training_plots, identifier, str(idx).zfill(4)))
    plt.clf()
    plt.close()
    return
...
def plot_trace(path_traces, identifier, xmax=250, final=False, dp=False):
    """
    """

    trace_path = '{}{}.trace.txt'.format(path_traces, identifier)
    da = read_table(trace_path, sep=' ')
    nrow = 3
    if dp:
        trace_dp_path = '{}{}.dptrace.txt'.format(path_traces, identifier)
...
#        ll_ticks = [-1.0*1e7, -0.5*1e7, 0]
    ...
    fig.savefig('{}_trace.png'.format(path_traces, identifier))
    fig.savefig('{}_trace.pdf'.format(path_traces, identifier))
    plt.clf()
    plt.close()
    return True
