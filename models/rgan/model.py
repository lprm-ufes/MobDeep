...
from differential_privacy.dp_sgd.dp_optimizer import dp_opt
...
def GAN_loss(Z, X, generator_settings, discriminator_settings, kappa, cond, CG, CD, CS, wrong_labels=False, model_parameters=None):
    if cond:
        # C-GAN
        G_sample = generator(Z, c=CG, **generator_settings)
        D_real, D_logit_real =  discriminator(X, c=CD, parameters=model_parameters, **discriminator_settings)
        ...
        # normal GAN
        G_sample = generator(Z, parameters=model_parameters, **generator_settings)
...
def discriminator(x, hidden_units_d, seq_length, batch_size, reuse=False, 
    ...
    with tf.variable_scope("discriminator") as scope:
        ...
        if parameters is None:
            ...
            b_out_D_initializer = tf.truncated_normal_initializer(value=parameters['discriminator/b_out_D:0'])
            lstm_initializer = tf.constant_initializer(value=parameters['discriminator/rnn/lstm_cell/kernel:0'])
            bias_start = tf.constant_initializer(value=parameters['discriminator/rnn/lstm_cell/bias:0'])
...
def dump_parameters(path_parameters, identifier, sess):
    """
    Save model parmaters to a numpy file
    """
    dump_path = '{}{}.npy'.format(path_parameters, identifier)
...
def load_parameters(path_parameters, identifier):
    """
    Load parameters from a numpy file
    """
    # FIXME: fix load parameters path
    load_path = '{}{}.npy'.format(path_parameters,identifier)
    model_parameters = np.load(load_path,allow_pickle=True).item()
    return model_parameters

# --- to do with trained models --- #

def sample_trained_model(settings, epoch, num_samples, Z_samples=None, C_samples=None, path_parameters=None):
    """
    ...
    parameters = load_parameters(path_parameters, settings['identifier'] + '_' + str(epoch))
